...
/**
* @fileOverview Grid控件源文件
*
* @修改标识：李玲20110810
* @修改描述：修改createEl方法，把参数autowidth替换为cellwidth，记录单元格宽度，修改时内部控件宽度根据此宽度生成
*/

/**
 * @class Grid控件，在jqgrid的基础上进行了修改，请参考<a target="_blank" href="http://www.secondpersonplural.ca/jqgriddocs/index.htm">jqgrid的api文档</a>
 * @name jqGrid
*/
...
(function ($) {
    ...
    $.fn.jqGrid = function (pin) {
		...
		isSafari = $.browser.webkit || $.browser.safari ? true : false;
            ...
            $("<div class='loading' id='load_" + this.id + "'><div class='loadingContent'>" + this.p.loadtext + "</div></div>").insertBefore(gv);
		...
		updatepager = function (rn, dnd) {
		    if (ts.p.pagerAlone) {
		        $(ts.p.pager).jhpager("refresh", ts.p.page, ts.p.records);
        ...
        setAlonePager = function (pgid) {
            $(pgid).jhpager({
                rowNum: ts.p.rowNum,
                async: true,
                beforeRequest: function (e, page) {
                    ts.p.page = page;
                    populate();
                    return false;
                }
		...
		td, ptr, tbody, imgs, iac = "", idc = "";
            ...
            imgs = null;
            $(this).append(thead);
            $("thead tr:first th", this).hover(function () { $(this).addClass('ui-state-hover'); }, function () { $(this).removeClass('ui-state-hover').removeClass('ui-jqgrid-th-focus'); }).mousedown(function () {
                $(this).removeClass('ui-state-hover').addClass('ui-jqgrid-th-focus');
            }).mouseup(function () {
                $(this).addClass('ui-state-hover').removeClass('ui-jqgrid-th-focus');
            });
            if (this.p.multiselect) {
                var emp = [], chk;
                        ...
                        $(ts.rows).each(function (i) {
                            if (i > 0) {
                                if (!$(this).hasClass("subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled')) {
                                    //李玲20110916 增加checkbox可以设置disabled属性，当为disabled时，在全选时不能选中
                                    var cbobj = $("#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + $.jgrid.jqID(this.id));
                                    if (!cbobj.attr("disabled")) {
		...
		hb = $("<div class='ui-jqgrid-hbox" + (dir == "rtl" ? "-rtl" : "") + "'></div>");
            ...
            if (ts.p.pager) {
                ...
                //修改代码 李玲20110921 下边的分页栏保持位置不变，不加到grid里
                if (ts.p.pagerAlone) {
                    $(ts.p.pager).css({ width: grid.width + "px" }).addClass('ui-jqgrid-pager');
                    setAlonePager(ts.p.pager);
                } else {
                    ...
                    if (ts.p.cellEdit === true) {
                        if (ts.p.multiselect && scb) {
                            ...
                            if (ts.p.savedRow.length > 0) {
                                if (ptr[0].rowIndex == ts.p.iRow && $.jgrid.getCellIndex(td) == ts.p.iCol) {
                                    return;
                                }
                                // save the cell
                            ...
                            if (ts.p.editError) { ts.p.editError = null; return; }
                            ri = ptr[0].rowIndex;
                        ...
                        if (ts.p.multiselect && ts.p.multiboxonly) {
                            ...
                            /*删除代码 李玲 20210203  当只使用checkbox选择行时，单击行的其他位置不改变选中状态
                            else {
                            ...
                            */
                            //添加代码 李玲
                                ...
                                if ($.isFunction(ts.p.onClickRow)) { cSel = ts.p.onClickRow.call(ts, ptr[0].id, e); }
                            }
                        } else {
                            $(ts).jqGrid("setSelection", ptr[0].id, true);
                        }
            ...
            if (rdata) {
				...
				air = $.isFunction(t.p.afterInsertRow) ? true : false;
                    while (k < datalen) {
                        ...
                        //李玲20110916 添加checkbox可以设置是否禁用
                        if (gi) {
                            var disabled = "";
                            if (data.multiSelectDisabled) {
                                disabled += "disabled='disabled'";
                            }
                            v = "<input role=\"checkbox\" type=\"checkbox\"" + disabled + " id=\"jqg_" + t.p.id + "_" + rowid + "\" class=\"cbox\"/>";
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		if(mopt.buttons.length > 0) {
			for(var i=0;i<mopt.buttons.length;i++) {
				...
				buttstr += "<a href='javascript:void(0)' id='"+mopt.buttons[i].id+"' class='fm-button ui-state-default'>"+mopt.buttons[i].text+"</a>";
			}
		}
		...
		cnt += c_b ? "<div class='ui-widget-content ui-helper-clearfix' style='text-align:"+mopt.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'><a href='javascript:void(0)' id='closedialog' class='fm-button ui-state-default'>"+c_b+"</a>"+buttstr+"</div>" :
...
// Form Functions
	createEl : function(eltype,options,vl,cellwidth, ajaxso) {
		var elem = "";
        var width = cellwidth - 4;
		...
		switch (eltype)
		{
			case "textarea" :
                ...
                //--修改后代码
                if(!options.cols) { $(elem).css({width:width-2});}
			...
			case "button" :
				...
				if(eltype != "button"){
                    ...
                    //--修改后代码
                    if(!options.size) { $(elem).css({width:width-2});}
				}
				$(elem).attr("role",role);
			break;
			case "image" :
...
// Date Validation Javascript
    ...
    //修改人 李玲2011/9/23 增加一个参数，要保存内容的td，用于判空后显示提示信息
	checkValues : function(val, valref,g, customobject, nam,td) {
			...
			if(edtrul.custom === true) {
				if( !(rqfield === false && this.isEmpty(val)) ) {
					if($.isFunction(edtrul.custom_func)) {
						var ret = edtrul.custom_func.call(g,val,nm,td);
						if($.isArray(ret)) {
							return ret;
						} else {
				...
				$('td',ind).each( function(i) {
					...
					if ( nm != 'cb' && nm != 'subgrid' && nm != 'rn') {
						...
						if(cm[i].editable===true) {
							...
							$(elc).addClass("editable").width($(this).width()-6); 
                            $(elc).click(function(){
		...
		if (editable==="1") {
			...
			$("td",ind).each(function(i) {
				...
				if ( nm != 'cb' && nm != 'subgrid' && cm.editable===true && nm != 'rn' && !$(this).hasClass('not-editable-cell')) {
					...
					cv = $.jgrid.checkValues(tmp[nm],i,$t,undefined,"",this);
					if(cv[0] === false) {
						cv[1] = tmp[nm] + " " + cv[1];
                        cv[2] = $(this);
						return false;
			...
			if (cv[0] === false){
				try {
                    //修改人 李玲20110922 修改错误提示方式
                    if($t.p.promptMode == "modal")
                    {
                        window.setTimeout(function(){$.jgrid.info_dialog($.jgrid.errors.errcap,cv[1],$.jgrid.edit.bClose);},100);                                    
                    }else if($t.p.promptMode == "tooltip"){
						cv[2].jhtooltip({message:cv[1]});                            
                    }      
                    cv[2].addClass("error");               
                    $t.p.editError = true; 
                        /*原代码    
...
//            if($t.p.editError){ return;}
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
                ...
                //添加代码 李玲20110810 添加单元格宽度
                var cellwidth = cc.width();
				var elc = $.jgrid.createEl(cm.edittype,opt,tmp,cellwidth,$.extend({},$.jgrid.ajaxOptions,$t.p.ajaxSelectOptions || {}));
                $(elc).click(function(){  
                    if($(elc).val() == "请填写" + $t.p.colNames[iCol]){
                        $(elc).val("");
                    }
                });
                ...
                //删除代码 李玲 2011-8-10 去掉不能编辑单元格的样式
				//cc.addClass("edit-cell");
				$($t.rows[iRow]).addClass("selected-row ui-state-hover"); 
		...
		return this.each(function(){
			...
			if(fr !== null) {
				...
				if (v2 !== $t.p.savedRow[fr].v){
                    ...
                    //修改人 李玲20110923 修改必填项的提示方式
					//var cv = $.jgrid.checkValues(v,iCol,$t);
                    var cv = $.jgrid.checkValues(v,iCol,$t,undefined,null,cc);
					if(cv[0] === true) {
						var addpost = {};
